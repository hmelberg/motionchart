...
# 
# In short:
# 1. Use a pandas dataframe
# 2. Define a motionChart, indicating the dataframe you want to use, which of the columns 
# you want to be the key variable (often a time variable which changes as the motion chart plays), 
# the x-variable, the y-variable and so on.
# 3. Display the chart in the browser, the notebook or save it as a html file 
#  
...
class SocrMotionChart(object):
    ...
    def htmlStringStart(self):
        ...
        # Change reference to boostrap.js file if the url is changed to  "custom-bootstrap.js"
        # The js available on SOCRs webpage lists this as boostrap.js, but on Git hub which many may use 
...
def sampleData():
    ...
    '''
    return house_price_vs_income

df = pd.read_json(sampleData())
