...
class SocrMotionChart(object):
    def __init__(self, 
        ...
        varLabels=None):
            ...
            varNamesString = ",".join(['"' + str(var) + '"' for var in varNamesList])
            ...
            SocrMotionChart.htmlString = socrTemplate.format(
                        ...
                        data = socrDataString,
                        url = url,
                        key = key, x = x, y = y, size = size, color = color, category = category,
    ...
    def to_clipboard(self):
        pyperclip.copy(SocrMotionChart.htmlString)
...
smc = SocrMotionChart(df, url = r"C:\Users\Hans\Google Drive\python\motionchart")
smc.to_browser()
...
df = df.set_index([timeVar,categoryVar])
df = df.reset_index()
gData = df.to_json(orient = 'records')
